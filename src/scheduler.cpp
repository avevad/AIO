...
inline BasicScheduler *BasicScheduler::IO::scheduler() {
  return &parent;
}
...
inline BasicScheduler::IO::IO(BasicScheduler &parent) : parent(parent) {
}

inline bool BasicScheduler::Timer::operator<(const Timer &timer) const {
  return when < timer.when;
}

inline BasicScheduler::BasicScheduler() : std_io(std::make_unique<StdIO>(StdIO::steal_from_system(fd_io))) {
}
...
inline BasicScheduler::IO &BasicScheduler::io() {
  return fd_io;
}
