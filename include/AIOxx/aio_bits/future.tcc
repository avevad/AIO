...
Future<Res> FutureBase<Res, Derived>::except(AsyncFunctor &&handler) {
  ...
  auto consumer = [promise = std::move(promise),
                   handler = std::forward<AsyncFunctor>(handler)](const MaybeResult<Res> &maybe_res) mutable {
    if (auto *error = std::get_if<std::exception_ptr>(&maybe_res)) [[unlikely]] {
      try {
        std::rethrow_exception(*error);
      } catch (Error &e) {
...
template<FutureResult Res, typename Derived>
FutureBase<Res, Derived>::FutureBase(FutureBase &&other) noexcept
    : BoundBase(std::move(other)), awaited(other.awaited), result(std::move(other.result)) {
  other.result.reset();
  other.awaited = true;
}
...
template<FutureResult Res, typename Derived>
PromiseBase<Res, Derived>::PromiseBase(PromiseBase &&other) noexcept
    : BoundBase(std::move(other)), fulfilled(other.fulfilled), consumer(std::move(other.consumer)) {
  other.consumer.reset();
  other.fulfilled = true;
}
...
Future<Res1> Future<Res>::then(AsyncFunctor &&fun) && {
  ...
  auto consumer = [promise = std::move(promise),
                   fun = std::forward<AsyncFunctor>(fun)](const MaybeResult<Res> &maybe_res) mutable {
    if (auto *res = std::get_if<WrappedResult<Res>>(&maybe_res)) {
...
Future<Res1> Future<Res>::map(Functor &&fun) && {
  ...
  auto consumer = [promise = std::move(promise),
                   fun = std::forward<Functor>(fun)](const MaybeResult<Res> &maybe_res) mutable {
    if (auto *res = std::get_if<WrappedResult<Res>>(&maybe_res)) {
      try {
...
Future<Res1> Future<void>::map(Functor &&fun) && {
  ...
  auto consumer = [promise = std::move(promise),
                   fun = std::forward<Functor>(fun)](const MaybeResult<void> &maybe_res) mutable {
    if (std::get_if<WrappedResult<void>>(&maybe_res)) {
      try {
        if constexpr (std::is_void_v<Res1>) {
          fun();
